...
final class SkillsCLIService {
    ...
    func openInteractiveInstall(source: String) -> CLICommandResult {
        ...
        let appleScript = """
        on run argv
            set npxPath to item 1 of argv
            set sourceArg to item 2 of argv
            set workingDir to item 3 of argv
            tell application "Terminal"
                activate
                do script "cd " & quoted form of workingDir & " && " & quoted form of npxPath & " --yes skills add " & quoted form of sourceArg
            end tell
        end run
        """
        ...
        process.arguments = ["-e", appleScript, executablePath, source, FileManager.default.currentDirectoryPath]
        ...
        do {
            ...
            return CLICommandResult(
                ...
                arguments: ["--yes", "skills", "add", source],
                workingDirectory: FileManager.default.currentDirectoryPath,
                stdout: "Opened Terminal for interactive install.",
                stderr: "",
                exitCode: 0,
                attemptedPaths: resolution.attemptedPaths
            )
